...
use bpst::monitoring::flamegraph;
...
fn build_fixture(
    ...
    let _guard = flamegraph::monitor().scoped_disable();
...
fn bench_dpdp(c: &mut Criterion) {
    ...
    for (label, share) in flamegraph::monitor().consensus_pressure_report() {
