...
",
    ...
    fn handle_dpdp_challenge(&mut self, data: &Value) -> CommandResponse {
        let _ctx = flamegraph::enter_context(Some(self.node_id.clone()), None::<String>);
        let _span = flamegraph::span(["dPDP", "handle_challenge"]);
    ...
    fn create_block(&mut self, height: usize, winning_proofs: Vec<BobtailProof>) {
        let _ctx = flamegraph::enter_context(Some(self.node_id.clone()), None::<String>);
        let _span = flamegraph::span(["CONSENSUS", "create_block"]);
        log_msg(
            "SUCCESS",
            "CONSENSUS",
            Some(self.node_id.clone()),
