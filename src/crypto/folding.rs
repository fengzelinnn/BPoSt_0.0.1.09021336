...
use crate::monitoring::flamegraph;
...
impl<F: PrimeField> RelaxedR1CSConstraint<F> {
    ...
    pub fn new(a: Vec<(usize, F)>, b: Vec<(usize, F)>, c: Vec<(usize, F)>, error: F) -> Self {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSConstraint::new"]);
        Self { a, b, c, error }
    }
}
...
impl<F: PrimeField> RelaxedR1CS<F> {
    ...
    pub fn is_satisfied(&self) -> bool {
        let _span = flamegraph::span(["Folding", "RelaxedR1CS::is_satisfied"]);
    ...
    pub fn witness_serialized(&self) -> Vec<Vec<u8>> {
        let _span = flamegraph::span(["Folding", "RelaxedR1CS::witness_serialized"]);
        self.witness
            .iter()
            .map(|value| {
...
impl<F: PrimeField> RelaxedR1CSBuilder<F> {
    /// 创建一个新的构建器。
    pub fn new() -> Self {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSBuilder::new"]);
        Self {
            // 预分配一个值为 1 的常量
    ...
    pub fn set_relaxation_parameter(&mut self, u: F) {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSBuilder::set_relaxation_parameter"]);
        self.u = u;
    }
    ...
    pub fn alloc_input(&mut self, value: F) -> usize {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSBuilder::alloc_input"]);
    ...
    pub fn alloc_witness(&mut self, value: F) -> usize {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSBuilder::alloc_witness"]);
        self.alloc_variable(value)
    }

    /// 分配一个常量值。
    pub fn alloc_constant(&mut self, value: F) -> usize {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSBuilder::alloc_constant"]);
    ...
    pub fn enforce_equal(&mut self, left: usize, right: usize) {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSBuilder::enforce_equal"]);
        self.constraints.push(RelaxedR1CSConstraint::new(
    ...
    pub fn enforce_boolean(&mut self, var: usize) {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSBuilder::enforce_boolean"]);
    ...
    pub fn mul(&mut self, left: usize, right: usize) -> usize {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSBuilder::mul"]);
    ...
    pub fn linear_combination(&mut self, terms: &[(usize, F)], constant: F) -> usize {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSBuilder::linear_combination"]);
    ...
    pub fn finish(mut self) -> RelaxedR1CS<F> {
        let _span = flamegraph::span(["Folding", "RelaxedR1CSBuilder::finish"]);
        let u = self.u;
...
pub fn dpdp_verification_relaxed_r1cs(
    ...
    let span = flamegraph::span(["Folding", "dpdp_verification_relaxed_r1cs"]);
    // 验证 dPDP 证明
...
pub fn block_validation_relaxed_r1cs(
    ...
    let span = flamegraph::span(["Folding", "block_validation_relaxed_r1cs"]);
    let mut builder = RelaxedR1CSBuilder::<Fr>::new();
...
pub fn state_update_relaxed_r1cs(
    ...
    let span = flamegraph::span(["Folding", "state_update_relaxed_r1cs"]);
    let mut builder = RelaxedR1CSBuilder::<Fr>::new();
...
impl<F: PrimeField> IncrementalRelaxedCircuit<F> {
    pub fn new() -> Self {
        let _span = flamegraph::span(["Folding", "IncrementalRelaxedCircuit::new"]);
        Self {
            steps: Vec::new(),
            accumulator: F::zero(),
        }
    ...
    pub fn absorb(&mut self, circuit: RelaxedR1CS<F>) {
        let _span = flamegraph::span(["Folding", "IncrementalRelaxedCircuit::absorb"]);
    ...
    pub fn merge(&mut self, other: &Self) {
        let _span = flamegraph::span(["Folding", "IncrementalRelaxedCircuit::merge"]);
    ...
    pub fn total_constraints(&self) -> usize {
        let _span = flamegraph::span(["Folding", "IncrementalRelaxedCircuit::total_constraints"]);
        self.steps
            .iter()
...
impl NovaFoldingCycle {
    pub fn new(storage_period: usize) -> Self {
        let _span = flamegraph::span(["Folding", "NovaFoldingCycle::new"]);
        // log_msg(
        //     "DEBUG",
        //     "NOVA",
        //     None,
    ...
    pub fn storage_period(&self) -> usize {
        let _span = flamegraph::span(["Folding", "NovaFoldingCycle::storage_period"]);
        self.storage_period
    }

    pub fn steps_completed(&self) -> usize {
        let _span = flamegraph::span(["Folding", "NovaFoldingCycle::steps_completed"]);
        self.steps
    }
    ...
    pub fn absorb_round(
        ...
        let span = flamegraph::span(["Nova", "absorb_round"]);
    ...
    pub fn finalize(&mut self) -> Result<Option<NovaFinalProof>, NovaFoldingError> {
        let span = flamegraph::span(["Nova", "finalize"]);
        log_msg(
            "DEBUG",
            "NOVA",
            None,
    ...
    pub fn verify_final_accumulator(
        ...
        let span = flamegraph::span(["Nova", "verify_final_accumulator"]);
        type NovaCompressed =
