...
impl FileOwner {
    ...
    pub fn dpdp_setup(&mut self, file_bytes: &[u8]) -> Vec<FileChunk> {
        let _ctx = flamegraph::enter_context(None::<String>, Some(self.owner_id.clone()));
        let _span = flamegraph::span(["dPDP", "owner_setup"]);
        // 1. 将文件字节流分割成原始数据块
    ...
    pub fn prepare_storage_request(
        ...
        let _ctx = flamegraph::enter_context(None::<String>, Some(self.owner_id.clone()));
        let _span = flamegraph::span(["dPDP", "prepare_request"]);
